...
The server binds `127.0.0.1` by default. Binding a non-loopback interface while `api_auth_enabled` is false is refused at startup, because that combination is an unauthenticated remote code-execution endpoint; `allow_unauthenticated_network_access: true` (or `TAKO_VM_ALLOW_UNAUTHENTICATED_NETWORK_ACCESS=true`) overrides it for deployments that authenticate in front of Tako VM.

The refusal applies to the host actually bound, not just the configured one: `tako-vm server --host 0.0.0.0` is checked against the same rule and exits non-zero rather than starting. This matters because the shipped container image's `CMD` passes `--host 0.0.0.0` — a container has to bind all interfaces to be reachable at all — so a containerized deployment must either enable authentication or set the opt-out explicitly. `docker-compose.yaml` takes the opt-out and publishes the API on `127.0.0.1:8000` only; widening that mapping without turning on `api_auth_enabled` and `api_keys` publishes an unauthenticated `/execute` endpoint.
